...
import numpy as np
import mujoco
from envs.flamingo_light_p_v0.manager.control_manager import ControlManager
from envs.flamingo_light_p_v0.manager.xml_manager import XMLManager
from envs.flamingo_light_p_v0.utils.math_utils import MathUtils
from envs.flamingo_light_p_v0.utils.mujoco_utils import MuJoCoUtils
from envs.flamingo_light_p_v0.utils.noise_generator_utils import truncated_gaussian_noisy_data, uniform_noisy_data
import glfw


class FlamingoLightPV0(MujocoEnv, utils.EzPickle):
    metadata = {"render_modes": ["human", "rgb_array", "depth_array"]}
    def __init__(self, config, render_flag=True, render_mode='human'):
        # Set Basic Properties
        self.id = "flamingo_light_p_v0"
        self.config = config
        self.action_dim = 4
    ...
    def _get_info(self):
        ...
        ang_vel = self.data.sensor('angular-velocity').data.astype(np.double)
        lin_vel = self.data.sensor("linear-velocity").data.astype(np.float32)
        cur_state = [dof_pos[0], dof_pos[1], dof_vel[2], dof_vel[3]]
