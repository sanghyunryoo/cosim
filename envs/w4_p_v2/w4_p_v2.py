...
import mujoco
import glfw
from envs.w4_p_v2.manager.control_manager import ControlManager
from envs.w4_p_v2.manager.xml_manager import XMLManager
from envs.w4_p_v2.utils.math_utils import MathUtils
from envs.w4_p_v2.utils.mujoco_utils import MuJoCoUtils
from envs.w4_p_v2.utils.noise_generator_utils import truncated_gaussian_noisy_data, uniform_noisy_data


class W4PV2(MujocoEnv, utils.EzPickle):
    ...
    def __init__(self, config, render_flag=True, render_mode='human'):
        ...
        qvel_joint_names = qpos_joint_names + ['FL_wheel_joint', 'FR_wheel_joint', 'RL_wheel_joint', 'RR_wheel_joint']
    ...
    def _get_obs(self):
        ...
        # geared legs
        dof_pos[8:12] = dof_pos[8:12] * self.gear_ratio 
        dof_vel[8:12] = dof_vel[8:12] * self.gear_ratio 

        ang_vel = self.data.sensor('angular-velocity').data.astype(np.double)
        lin_vel = self.data.sensor("linear-velocity").data.astype(np.double)
    ...
    def step(self, action):
        ...
        pos_leg = dof_pos[8:12] * self.gear_ratio

        vel_hip = dof_vel[0:4]
        vel_shoulder = dof_vel[4:8]
        vel_leg = dof_vel[8:12] * self.gear_ratio 
        ...
        leg_torques = leg_torques * np.full(4, self.gamma, dtype=np.float64)
    ...
    def _get_info(self):
        ...
        joint_state = [dof_pos[0], dof_pos[1], dof_pos[2], dof_pos[3], dof_pos[4], dof_pos[5], dof_pos[6], dof_pos[7],
                        dof_pos[8],dof_pos[9], dof_pos[10], dof_pos[11], dof_vel[12], dof_vel[13], dof_vel[14], dof_vel[15]]
