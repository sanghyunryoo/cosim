...
class MuJoCoUtils:
    ...
    def init_heightmap_visualization(self, x_res, y_res):
        """
        ...
        It finds all site IDs whose names follow the pattern "heightmap_site_i_j" for i in [0, y_res) and j in [0, x_res).
        The result is stored in self.site_ids as a 2D list of shape [y_res][x_res], where each entry is the integer site ID
        corresponding to that grid cell.

        Args:
            x_res (int): Number of columns in the heightmap grid.
            y_res (int): Number of rows in the heightmap grid.

        Raises:
            ValueError: If any expected site name is not found in the model's XML.
        """
        self.site_ids = [[None for _ in range(x_res)] for _ in range(y_res)]
    ...
    def get_height_map(self, data, x_size, y_size, x_res, y_res):
        """
        Generate a heightmap by raycasting from the robot's base frame onto the ground.

        For each grid cell in a (x_res × y_res) window centered on the robot, this function:
        ...
        Args:
            ...
            x_size (float): Width of the heightmap window in meters (robot’s local frame).
            y_size (float): Depth of the heightmap window in meters (robot’s local frame).
            x_res (int): Number of columns (sampling points) along the x dimension.
            y_res (int): Number of rows (sampling points) along the y dimension.

        Returns:
            numpy.ndarray: A 1D array of length (x_res * y_res), containing the height difference
        ...
        if self.site_ids is None:
            raise RuntimeError(
                "Heightmap visualization sites not initialized. Call init_heightmap_visualization(x_res, y_res) first."
            )
        ...
        x_min_robot, x_max_robot = -x_size / 2, x_size / 2
        y_min_robot, y_max_robot = -y_size / 2, y_size / 2
        num_x, num_y = x_res, y_res
